-- Fix sync_profile_seller_payout_readiness: stripe_requirements_currently_due is jsonb (0027),
-- not text[]. Migration 0057 incorrectly used array_agg → text[].
  ...
  set
    ...
    stripe_requirements_currently_due = case
      when v_patch ? 'stripe_requirements_currently_due'
        then coalesce(v_patch->'stripe_requirements_currently_due', '[]'::jsonb)
    ...
    stripe_requirements_disabled_reason = case
      when v_patch ? 'stripe_requirements_disabled_reason'
        then case
          when v_patch->'stripe_requirements_disabled_reason' is null
            or jsonb_typeof(v_patch->'stripe_requirements_disabled_reason') = 'null'
            then null
          else nullif(v_patch->>'stripe_requirements_disabled_reason', '')
        end
    ...
    seller_verified_at = case
      when v_patch ? 'seller_verified_at'
        then case
          when v_patch->'seller_verified_at' is null
            or jsonb_typeof(v_patch->'seller_verified_at') = 'null'
            then null
          else (v_patch->>'seller_verified_at')::timestamptz
        end
    ...
    seller_rejection_reason = case
      when v_patch ? 'seller_rejection_reason'
        then case
          when v_patch->'seller_rejection_reason' is null
            or jsonb_typeof(v_patch->'seller_rejection_reason') = 'null'
            then null
          else nullif(v_patch->>'seller_rejection_reason', '')
        end
...
comment on function public.sync_profile_seller_payout_readiness is
  'Worker-only: patch seller onboarding + Stripe status fields (jsonb requirements array).';
