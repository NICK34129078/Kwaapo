-- Feed accessible fallback:
-- 1) Refresh may re-surface recently viewed posts (TikTok-style home refresh).
-- 2) Explore fallback includes untagged public posts (not only tagged).
