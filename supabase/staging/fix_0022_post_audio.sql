-- Staging-only fix: migration 0022 has invalid "drop constraint ... on table" syntax on fresh Postgres.
-- Production applied via dashboard; this file is for kwaapo-staging-checkout bootstrap only.
...
alter table public.posts drop constraint if exists posts_audio_source_check;

alter table public.posts
  add constraint posts_audio_source_check
  check (audio_source in ('none', 'user_upload', 'app_library', 'external'));

alter table public.posts drop constraint if exists posts_audio_start_ms_check;

alter table public.posts
  add constraint posts_audio_start_ms_check
  check (audio_start_ms >= 0);

alter table public.posts drop constraint if exists posts_audio_volume_check;
...
create policy "Public read post audio"
  on storage.objects for select using (bucket_id = 'post-audio');
